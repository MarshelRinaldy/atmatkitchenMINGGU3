            ...
            <div class="row" style="margin-left: 80px; margin-top: 40px;">
                <div class="col-6">
                    <button class="btn-search" style="margin-right: 10px;"
                        onclick="window.location.href='{{ route('create_BahanBaku') }}'">Tambah Bahan Baku</button>
                </div>
                <div class="col-6">
            ...
            <table>
                ...
                {{-- @foreach ($bahanBaku as $index => $bahan)
                    <tr>
                        ...
                        <td>{{ $bahan->stok_bahan_baku }}</td>
                        <td>{{ $bahan->harga_bahan_baku }}</td>
                        <td>{{ $bahan->tanggal_kadaluarsa }}</td>
                        <td>
                            <form action="{{ route('edit_bahan_baku', $bahan) }}" method="get">
                                <button type="submit" class="btn-search">Update</button>
                            </form>
                        </td>
                        <td>
                            <form action="{{ route('delete_bahan_baku', $bahan) }}" method="post">
                                @method('delete')
                                @csrf
